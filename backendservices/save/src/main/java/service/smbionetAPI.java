    ...
    public Response process(String input) {
        ...
        try {
            switch (EVENT.valueOf(obj.getString("event"))) {
                case GETALL:
                    return Response.ok().entity(getAll(obj).toString(INDENT_FACTOR)).build();
                case GET:
                    return Response.ok().entity(get(obj).toString(INDENT_FACTOR)).build();
                case SAVE:
                    return Response.ok().entity(save(obj).toString(INDENT_FACTOR)).build();
                case PURGE:
